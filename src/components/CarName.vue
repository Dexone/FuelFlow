<template>
    ...
    <div class=" mt-4" v-if="userStore.userCost != 0">
        ...
        <div class="flex font-semibold text-gray-900 text-2xl items-center w-full">

            {{ userStore.userCar }}

            <!-- edit -->
            <div class="absolute bg-white w-48 max-w-24" v-if="showEditCar === false">
                <div class="relative">
                    <input v-model="car"
                        class="block w-full  text-sm text-gray-900 border-b-2 focus:outline-none focus:border-blue-600"
                        :placeholder="userStore.userCar">
                    <button class="text-black absolute end-0 bottom-0  font-medium rounded-lg text-sm py-1"
        ...
        <p class="text-gray-500 text-xs" v-if="userStore.userInfo.length < 1">Недостаточно данных</p>
        <p class="text-gray-500 text-xs" v-if="userStore.userInfo.length >= 1"> {{ new
